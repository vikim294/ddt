import { MAP_BOUNDARY_GAP, PLAYER_BOUNDING_BOX_LENGTH, TRIDENT_ANGLE_DIFFERENCE } from "../../constants.js"
import { SHELL_CRATER_50_round } from "../../shellCraters.js"
...
const prettyMapCanvasEl = document.querySelector('#prettyMap')
...
prettyMapCanvasEl.width = CANVAS_WIDTH
prettyMapCanvasEl.height = CANVAS_HEIGHT
...
const prettyMapCtx = prettyMapCanvasEl.getContext('2d')
...
function drawPrettyMap() {
    prettyMapCtx.clearRect(0, 0, CANVAS_WIDTH, CANVAS_HEIGHT)
    prettyMapCtx.drawImage(imgEl, 0, 0)
    prettyMapCtx.save()
    prettyMapCtx.globalCompositeOperation = 'destination-in'
    prettyMapCtx.drawImage(mapCanvasEl, 0, 0)
    prettyMapCtx.restore()
}
...
function drawPlayer() {
    ...
    if(players[activePlayerIndex].direction === 'right') {
        ...
        throw new Error(`角色朝向错误, ${players[activePlayerIndex].direction}`)
    }
...
function getSurfacePoints(x, y) {
    const res = []
    const { data } = mapCtx.getImageData(x - PLAYER_BOUNDING_BOX_LENGTH / 2, y - PLAYER_BOUNDING_BOX_LENGTH / 2, PLAYER_BOUNDING_BOX_LENGTH, PLAYER_BOUNDING_BOX_LENGTH)
    for(let col = 0; col < PLAYER_BOUNDING_BOX_LENGTH; col++) {
        for(let row = 0; row < PLAYER_BOUNDING_BOX_LENGTH; row++) {
            const index = (row * PLAYER_BOUNDING_BOX_LENGTH + col) * 4
            // console.log(data[index])
            const r = data[index]
            const g = data[index + 1]
            if(r === 255) {
                res.push({
                    x: col + x - PLAYER_BOUNDING_BOX_LENGTH / 2 + 1,
                    y: row + y - PLAYER_BOUNDING_BOX_LENGTH / 2 + 1,
                })
                break
            } 
            else if(g === 255) {
...
function calculatePlayerData(player) {
    ...
    // 新
    const surfacePoints = getSurfacePoints(player.x, player.y)
...
function drawBomb() {
    ...
    if(elapsedSec >= players[activePlayerIndex].bomb.bombSec) {
...
function bombTarget(target, ctx) {
    ...
    mapCtx.translate(x - damageRadius, y - damageRadius)
